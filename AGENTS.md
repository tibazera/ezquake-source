...
**Itens 2 (MSAA) e 3 (gamma real) não implementados nesta sessão** — ver análise detalhada acima ("Conclusão prática"): exigem a mesma peça de arquitetura nova (render target offscreen + segunda passagem de composição pro swapchain), que não cabe como ajuste incremental pequeno e arrisca regressão em todos os pipelines que desenham na render pass principal. Ainda recomendado: implementar primeiro a abstração mínima de render-target offscreen (tipo `VK_RT_Begin`/`VK_RT_End` do FTEQW) numa sessão dedicada, e depois pendurar MSAA e gamma nela.
