...
- **Vulkan nunca gera mip levels** (nenhuma textura tem `mipLevels > 1` em `VK_TextureCreate2D`/`vk_texture.c`; sampler usa `minLod=maxLod=0`). Isso significa que o menu "Quality Mode" (`gl_texturemode`, ex. `GL_LINEAR_MIPMAP_LINEAR` vs `GL_LINEAR`) só tem efeito parcial sob Vulkan: nearest-vs-linear é respeitado (`VK_TextureSetFiltering` já funciona e já era chamado corretamente via `R_TextureModeForEach`), mas as variantes com mipmap são indistinguíveis das sem mipmap (sempre mip 0). Não é regressão desta sessão, é uma lacuna pré-existente do port Vulkan. Implementar geração de mipmap real (blit chain ou compute) é trabalho separado, maior que "fix pequeno".
