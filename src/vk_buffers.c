...
// geometry, worse at higher framerates since frames overlap more). Both
// vkQuake (NUM_DYNAMIC_BUFFERS) and FTEQW (per-frame vkframe->dynbufs) use
// the same fix: duplicate the dynamic buffer storage per frame-in-flight and
// select the live copy with the same index used for that frame's fence.
...
static void VK_BufferShutdown(void)
{
	...
	// showed up as a full driver TDR on vid_restart during a live session on
	// desktop, not just on the boot-time no-op case where nothing was ever
	// in flight.
	if (vk_options.logicalDevice != VK_NULL_HANDLE) {
		vkDeviceWaitIdle(vk_options.logicalDevice);
	}
